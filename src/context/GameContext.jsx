...
import { createEmptyBoard, placeShipsRandomly, attackCell, checkGameOver, isShipSunk, canPlaceShip, placeShip } from "./utils/boardUtils";
...
export const GameProvider = ({ children }) => {
  ...
  // Game phase state
  const [gamePhase, setGamePhase] = useState(savedState?.gamePhase || GAME_PHASES.SETUP);
  const [isGameActive, setIsGameActive] = useState(savedState?.isGameActive || false);
  const [winner, setWinner] = useState(savedState?.winner || null);
  const [gameMode, setGameModeState] = useState(savedState?.gameMode || GAME_MODES.NORMAL);
  const [gameStartTime, setGameStartTime] = useState(savedState?.gameStartTime || null);
  
  // Board state
  const [playerBoard, setPlayerBoard] = useState(savedState?.playerBoard || createEmptyBoard());
  const [aiBoard, setAiBoard] = useState(savedState?.aiBoard || placeShipsRandomly());
  
  // Ship state
  const [shipsToPlace, setShipsToPlace] = useState(savedState?.shipsToPlace || [...SHIPS]);
  ...
  const [aiTargetDirection, setAiTargetDirection] = useState(savedState?.aiTargetDirection || null);
  ...
  const placePlayerShip = useCallback((row, col) => {
    ...
    if (canPlaceShip(newBoard, row, col, selectedShip.size, direction)) {
      ...
      setShipsToPlace(shipsToPlace.filter(s => s.id !== selectedShip.id));
  ...
  const determineAiAttack = useCallback(() => {
    ...
    if (aiHitStack.length > 0) {
      ...
      if (aiTargetDirection) {
        const dirPositions = surroundingPositions.filter(pos => 
          (pos.dr === aiTargetDirection.dr && pos.dc === aiTargetDirection.dc) ||
          (pos.dr === -aiTargetDirection.dr && pos.dc === -aiTargetDirection.dc)
        );
      ...
      if (surroundingPositions.length > 0) {
        const targetPos = surroundingPositions[
  ...
  }, [gameMode, isGameActive, determineAiAttack, playerBoard, aiBoard, aiTargetDirection, aiHitStack, aiLastHit, updateBoards]);

  // Player attack
  const playerAttack = useCallback((row, col) => {
  ...
  }, [isGameActive, playerAttackedPositions, aiBoard, playerBoard, updateBoards, aiTurn]);
  ...
  useEffect(() => {
    if (gamePhase === GAME_PHASES.SETUP && shipsToPlace.length === 0 && placedShips.length === SHIPS.length) {
      setGamePhase(GAME_PHASES.PLAYING);
      setIsGameActive(true);
      setGameStartTime(Date.now());
    }
  ...
  useEffect(() => {
    ...
    gamePhase, playerBoard, aiBoard, isGameActive, winner, 
    gameMode, gameStartTime, playerAttackedPositions,
    aiAttackedPositions, aiLastHit, aiTargetDirection,
    aiHitStack, shipsToPlace, placedShips
  ]);
