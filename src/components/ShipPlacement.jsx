  ...
  const handleCellClick = (row, col) => {
    if (selectedShip) {
      ...
      if (!placed) {
        ...
        console.error("Cannot place ship at this position");
      }
    }
  };
  ...
  return (
    <div className="ship-placement-container">
      ...
      <div className="placement-instructions">
        <p>Click on a ship from the list, then click on the board to place it. Ships can only be placed horizontally.</p>
        <button 
          className="reset-placement-button"
      ...
      <div className="placement-area">
        <div className="ships-container">
          ...
          {shipsToPlace.length === 0 ? (
            <p className="all-placed-message">All ships placed! Game will start automatically.</p>
          ) : (
          ...
          <div className="placed-ships">
            ...
            <ul className="placed-ships-list">
              {placedShips.map((ship, index) => (
                <li key={index}>
                  {ship.type} at position ({ship.position.row}, {ship.position.col})
                </li>
              ))}
            </ul>
          </div>
                ...
                {row.map((cell, colIndex) => (
                  <div 
                    ...
                    className={`board-cell ${cell ? 'occupied' : 'empty'} ${
                      selectedShip && canPlaceShip(playerBoard, rowIndex, colIndex, selectedShip.size, "H") ? 'valid-placement' : ''
                    }`}
